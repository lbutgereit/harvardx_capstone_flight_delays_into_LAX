...
## Filter data and selecting columns
...
of the airline industry as a whole.  However, this may not be useful in predicting arrival
...
It is clear from \@ref(fig:average-delay-by-airline) that the airline itself could
be a predictor of the arrival delay.  That means that the airlie could be influential
...
the same showing a near linear relationship between departure delay and arrival delay.  This 
leads the author to believe that perhaps two models must be created:  1) one that attempts 
...
displayed in Table \@ref(tab:ORD-LAX), the total *AirTime* is in near 270 minutes.  But if
...
delays during the evening, night, and early morning hours.  This could be an important
predictor of flight delays.
...
about the actual flight crew involved in the flight.  The tail number (which
identifies the physical plane being used).

These two fields were omitted as is explained in more detail in Sub-Section \@ref(iteration-1).
...
\@ref(tab:ImportantColumns).  For the first attempt, the notebook used had 4 Gig of swapspace.
The building of the *rpart* model crashed.  The author increased swapspace to 8 Gig.  The
*rpart* model built for a substantial amount of additional time but still crashed.  The
author increased the swapspace again to 16 Gig and the model still crashed.  On investigation
the tail number included LB unique factor and the flight number included LB unique factor.
The author believes that the number of factors was too high for the compute environment
available to the author.  Those two fields were removed


These four models were *glm* (Generalized Linear Model), *rpart* (a decision tree), *mlp* 
(a multilayer peceptron), and *knn* (K nearest neighbour).  These were run using default 
values using two iterations each.  This gave the author an indication of which model 
might be the most effective.

This investigation took some time and the code to execute this is not *executable* in 
this document but
...
rm(rpart_fit, glm_fit, rpart_cm, glm_cm, fit, cm)
```
