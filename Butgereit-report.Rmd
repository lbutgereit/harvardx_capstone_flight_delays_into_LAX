...
Value <- c(nrow, sum_late, sum_late / nrow, sum_not_late, sum_not_late / nrow)
df <- data.frame(Measurement, Value)
...
However, this project is attempting to predict whether or not a flight will be late *before* it takes off.  Therefore,
the departure delay field will be omitted.
...
## Superficial loop through a number of models
...
These four models were *glm* (Generalized Linear Model), *rpart* (a decision tree), *mlp* (a multilayer peceptron),
and *knn* (K nearest neighbour).  These were run using default values using three iterations each.  This gave
the author an indication of which model might be the most effective.
