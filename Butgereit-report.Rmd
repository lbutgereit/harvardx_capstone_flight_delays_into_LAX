...
number of different strategies which were investigated.  
For each
...
```{r, average-delay-by-departure-delay, fig.cap="Average arrival delay by departure delay", warnings=FALSE}
...
```{r, average-delay-by-departure-delay-extended, 
	fig.cap="Average arrival delay by departure delay extended times", warnings=FALSE}
arrival_delay_graph(
	x = flights_by_departure_delay$DepDelay, 
	y = flights_by_departure_delay$average_delay,
	data=flights_by_departure_delay) +
...
```{r, average-delay-by-airtime, fig.cap="Average arrival delay by airtime flown", warnings=FALSE}
...
```{r, average-delay-by-arrival-time, fig.cap="Average arrival delay by arrival time", warnings=FALSE}
...
From this exploration, it is assumed that fields which predict a delay in arrival times also 
predict whether or not a flight will be late.  It is clear that the fields indicated in 
Table \@ref(tab:ImportantColumns) are important
...
section except at the end to test the final model.  That is described in Sub-Section \@ref(iteration_4).
...
\@ref(tab:ImportantColumns).  For the first attempt, the notebook used had 4GB of swapspace.
...
author increased the swapspace again to 16GB and the model still crashed.  On investigation
...
the *mlp* model and the *knn* model, the author chose a range of values to try.  Again, the
...
## Iteration 4 - Final model with *validation* dataset {#iteration_4}

Iteration 3 shows that the *rpart* algorithm with a *cp* value of 5e-04 provided the best accuracy values.
That selection of model and parameters are used in Iteration 4 with the *validation* set to create
the final model.
...
However, even with the memory limitations, the fnal model achieved an accuracy of `r rpart_cm$overall[1]` with a
Sensitivity of `r rpart_cm$byClass[1]` and a Specificity of `r rpart_cm$byClass[2]`.  
it is firmly believed that if a server were sourced for this project (and not a notebook), better
