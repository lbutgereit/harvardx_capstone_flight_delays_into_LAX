...
predict average arrival delays of airflights and to classify a flight as being either on-time or
...
during the period of time 1995 to 2020.  From this data this project attempts to answer the question
...
methods and models to be able to predict the arrival delay time of a flight and to predict
whether the flight was on-time or late.  There were a
number of different strategies which were investigated.  For each model which attempted to
predict the arrival delay time, the RMSE (root mean squared error) is provided.  For each
model which attempted to predict whether a flight was on-time or late, the accuracy, sensitivity,
and specificity is provided. 

A summary of the results can be found in Section \@ref(results).
...
There were memory issues and an additional 16G swap space was configured on the notebook with the following Linux commands executed as superuser:
...
**NB:**  In retrospect, even with 16G of swapspace, some of the models which the author
wished to run aborted for lack of memory.  For the models that did run, 8G of swapspace
was sufficient
...
#
build_late_models_iteration_1 <- FALSE
build_late_models_iteration_2 <- TRUE
build_late_models_iteration_3 <- FALSE
...
specifically at the field *late* which was added by the author and indicates whether or not the flight
was late.
...
### Arrival delays through the years
...
bar_graph(x = flights_per_year$Year, y = flights_per_year$number_of_flights,
  ...
  scale_x_continuous(limits=c(min(non_validation$Year)-1, max(non_validation$Year)+1), 
...
Figure \@ref(fig:average-delay-by-year) may show an over all trend of decreasing average
arrival delays over the years.  However, this may not be useful in predicting arrival
delays in a new year.  It will have to be investigated
...
arrive late.
This relationship can be seen in Figure \@ref(fig:average-delay-by-departure-delay).

```{r, average-delay-by-departure-delay, fig.cap="Average arrival delay by departure delay"}
...
arrival_delay_graph(
  ...
  scale_x_continuous(limits=c(mean(non_validation$ArrDelay) - sd(non_validation$ArrDelay),
                              mean(non_validation$ArrDelay) + sd(non_validation$ArrDelay)))

```

The horizontal and vertical scales on Figure \@ref(fig:average-delay-by-departure-delay) are the same showing
a near linear relationship between departure delay and arrival delay.  This leads the author to believe that
perhaps two models must be created:  1) one that attempts make predictions before the flight departs and 2) and
one that attempts to make predictions immediately after the flight departs taking into account any
departure delays.

The limits of the horizontal and vertical axes can changed to include all arrival delays and 
departure delays.  This can be 
seen in \@ref(fig:average-delay-by-departure-delay-extended)

```{r, average-delay-by-departure-delay-extended, fig.cap="Average arrival delay by departure delay extended times"}
arrival_delay_graph(
    x = flights_by_departure_delay$DepDelay, 
    y = flights_by_departure_delay$average_delay,
    data=flights_by_departure_delay) +
  xlab("Departure delay (minutes)") +
  scale_y_continuous(limits=c(min(non_validation$ArrDelay), max(non_validation$ArrDelay)))
  scale_x_continuous(limits=c(min(non_validation$DepDelay), max(non_validation$DepDelay)))
...
Figure \@ref(fig:average-delay-by-departure-delay-extended) shows that even with flights which had the maximum
...
displayed in Table \@ref(tab:ORD-LAX), the total *AirTime* is approximately equal.  But if
you subtract the *DepTime* from the *ArrTime*, there is only approximately 2 hours.  Therefore,
...
This sub-section specifically looks at boolean field *late* which is one if the flight was late (arrived later than scheduled)
...
nrow <- nrow(non_validation)
sum_late <- sum(non_validation$late == 1)
sum_not_late <- sum(non_validation$late != 1)
percent_late <- sum_late / nrow
percent_not_late <- sum_not_late / nrow
...
From the aforegoing investigations into which fields or columns affect the average arrival delay, it can be assumed
that those same columns are predictors for whether or not a flight arrival will be late.
...
From these fields, two were omitted from the models which investigation.  The flight number provides
the same type of information as the airport of origin and can be omitted.  The tail number (which
identifies the physical plane being used) had too many elements and the notebook where this model
was developed couldn't handle the sizes.  This is explained in more detail in Sub-Section LB
...
section except at the end to test the final model.  That is described in section LB

There were four iterations of model development.  
## Iteration # 1 - Superficial loop through a number of models

The first attempt to find a model to predict whether or not a flight will be late involved a superficial
run through a handful of different models using default values.  Four different models were chosen because
their were *structurally* different.  The four different models attempted to solve the problem in completely
different ways.

These four models were *glm* (Generalized Linear Model), *rpart* (a decision tree), *mlp* (a multilayer peceptron), and *knn* (K nearest neighbour).  These were run using default values using two iterations each.  This gave the author an indication of which model might be the most effective.
...
if ( build_late_models_iteration_1 ) {
        load(train_rda)
        load(test_rda)
        models <- c("rpart", "glm", "mlp")              # knn didn't return within 12 hours
...
The *knn* model did not complete after 12 hours.  The information for the other three models
is presented in Table \@ref(tab:iteration-1-model-results).


```{r, eval=TRUE, echo=FALSE}
Model <- c("rpart", "glm", "mlp")
Best Parameter <- c("cp = 0.0008950666", "NA", "size = 1")
Accuracy <- c(0.7589, 0.7622, 0.565)
Sensitivity <- c(0.9329, 0.9125, 1.000)
Specificity <- c(0.5329, 0.5670, 0.000)
df <- data.frame(Field, Value)
...
This information informs the author that the *rpart* model and *glm* model performed similarly.  Better results
...
## Itereation # 2 - Loop through models with tuning parameters

The second iteration on attempting to predict whether or not a flight was lates used tuning
parameters for the four models.  In the case of the *rpart* model and the *glm* model, parameters
in the vicinity of the parameters provided in the first iteration were used.  In the case of
the *mlp* model and the *knn* model, the author chose a range of values to try.
...
if ( build_late_models_iteration_2 ) {
        ...
        tuning_parameters <- c(
                        ...
                        data.frame(size = seq(3, 23, 5)))               # mlp
...
```{r, eval=TRUE, echo=FALSE}
Model <- c("rpart", "glm", "mlp")
Best Parameter <- c("cp = 5e-04", "NA", "size = 3")
Accuracy <- c(0.7626, 0.7622, 0.565)
Sensitivity <- c(0.9092, 0.9125, 1.000)
Specificity <- c(0.5722, 0.5670, 0.000)
df <- data.frame(Field, Value)
...
This information informs the author that the *rpart* model and *glm* model performed similarly.
Those two models will be investigated in more detail in Iteration 3.
...
```r, eval=TRUE, echo=FALSE}
Model <- c("rpart", "glm")
Best Parameter <- c("cp = 8e-04", "NA")
Accuracy <- c(0.7618, 0.7622)
Sensitivity <- c(0.9176, 0.9125)
Specificity <- c(0.5595, 0.5670)
df <- data.frame(Field, Value)
...
## Iteration 4 - Final model with *validation* dataset

As a result of Iteration 3, the *rpart* algorithm with a *cp* value of 5e-04 provided the best accuracy values.
...
The results of Iteration 4 can be seen in Table 

```r, eval=TRUE, echo=FALSE}
Model <- c("rpart")
Parameter <- c("cp = 5e-04")
Accuracy <- c(0.7598)
Sensitivity <- c(0.8963)
Specificity <- c(0.5825)
df <- data.frame(Field, Value)
